...
class Combination {
  ...
  public void combination(ArrayList<Position> arr, int index, int target){
    if(index == r){
    ...
    if(target == n) return;
    now[index] = target;
    combination(arr, index+1, target+1);
    combination(arr, index, target + 1);
  }
